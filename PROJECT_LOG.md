...
- **Branding:** The *Vaultly by PBSS* identity is applied across every surface, and all remaining user-facing `Paperless-ngx` product names have been replaced with `Vaultly`.
- **Built:** `vaultly-paperless:2.20.15-frontend9` is prepared locally and not yet built on the VM. The patch now covers 41 files and is generated mechanically with `git diff` against the pinned upstream commit `05e48b23166df7c7afe6f329b460b0511a89496c`; `git apply --check` exits 0.
- **Pushed:** `origin/main` includes the branding assets and the `frontend9` revision. No VM secret or local credential file is stored in Git.
- **VM:** `Ubuntu-Vaultly` is persistent and stable. PostgreSQL, Redis, Tika, and Gotenberg remain running; the webserver is stopped after the `frontend7` build. `frontend8` was never built on the VM and is superseded by `frontend9`.
- **Next:** Update `PAPERLESS_APP_LOGO` in the VM's `.env` to `/static/vaultly-lockup-full.svg`, build `frontend9`, start the webserver, then verify the login page, sidebar, favicon, splash screen, upload toast, and PDF guide cover.

## History

### 2026-09-12 — Removed the remaining Paperless-ngx product names (frontend9)

- The user reported an upload notification reading "Document ... was added to Paperless-ngx." Rather than fix that one string, cloned the pinned upstream source and swept it for every user-facing occurrence of the product name.
- Found and replaced 31 strings. Beyond the reported toast: the processing toast and drag-and-drop hint; the configuration, personal-settings, system-status, workflows and mail-rule screens; `environment.appTitle`, which is the fallback used before the server's `APP_TITLE` setting loads; all eleven account page titles (sign up, four password-reset stages, MFA, three social-account pages, account inactive); the transactional email body; `EMAIL_SUBJECT_PREFIX` and `ACCOUNT_EMAIL_SUBJECT_PREFIX`, which set the subject line on messages the server sends; `MFA_TOTP_ISSUER`, which is the name shown beside the account in a user's authenticator app; and the API schema title plus the version and system-status endpoint descriptions.
- Found and fixed a packaging gap the sweep exposed. Thirteen of the changed backend files were never copied into the runtime image by `Dockerfile.vaultly`, so the edits would have been applied to the source and then silently dropped at build time — the Django templates and Python changes would have had no effect. The Dockerfile now copies the whole `src/documents/templates/` tree instead of three individual templates, plus `settings.py`, `serialisers.py`, and `documents/views.py`. This gap predates this change and would have affected any future backend template work.
- Changed how the patch is maintained. It is now regenerated with `git diff` against a clean checkout of the pinned commit rather than hand-edited, which removes the risk of miscounted hunk headers. It covers 41 files and `git apply --check` exits 0 on a clean tree.
- `MFA_TOTP_ISSUER` is display-only in the `otpauth` URI, so existing two-factor enrolments continue to work; the issuer label updates for new enrolments.
- Deliberately left unchanged: `name="author"` meta tags, GitHub and `docs.paperless-ngx.com` links, licence and copyright notices, internal identifiers such as `PaperlessTask` and `PaperlessConfig`, logger names, log lines, temp-file prefixes, and template paths. Two strings in the update-check footer also keep the upstream name because the existing patch marks that block `d-none`, so it never renders.
- Reason: the product should present one consistent name to users, including in surfaces that are easy to miss such as email subjects and authenticator apps, while preserving upstream attribution and licence obligations.

### 2026-09-11 — Branding: Vaultly by PBSS identity applied

- Inventoried every branding surface against the repository rather than from memory: login logo and title, sidebar brand, favicon (Angular and the Django `/favicon.ico` route), PWA manifest, app-loading splash, Django administration labels, guide cover, PDF, welcome widget, and the READMEs.
- Confirmed the supplied PBSS assets. They are named `pbss-badge-white.png` and `pbss-badge-white-ring.png`, not the `pbss-logo*.png` names in the brief; both are 200x200 RGBA with fully transparent corners and no halo. Used as supplied, under their actual names, to avoid duplicating binaries in Git.
- Refined the core mark. One 64-unit construction now drives every asset, so the favicon and the logo are the same drawing. The four index marks became notches cut into the ring via a stroke dash pattern: they read as dial machining at display sizes and close optically below about 24 px, so the mark simplifies to a clean ring in a browser tab instead of fragmenting. Added indigo and teal gradients, rebuilt the V as a filled taper with flat terminals and a pointed apex, and removed both the indigo dot that merged into a blob at 16 px and the stray teal circle after the wordmark.
- Rebuilt the wordmark as outlined Inter paths (`wght 650`, `opsz 32`, tracked -1.2%). This was the largest defect in the previous mark: the logo is served through an `<img>` tag, so its live SVG `<text>` element resolved fonts on the *viewer's* machine and rendered in Inter, Segoe UI, or Arial depending on who opened it. Inter is SIL OFL 1.1, which permits embedding outlines.
- Built full and compact lockups. `BY PBSS` is set as a superscript at cap 9.5 against the wordmark's cap 34, its cap aligned to the top of the wordmark, with the partner badge behind a hairline rule — so PBSS reads as attribution and never as a second logo.
- Embedded the PBSS badge in the lockups as a base64 data URI. This is required rather than convenient: an SVG loaded through an `<img>` tag cannot fetch external images, and `PAPERLESS_APP_LOGO` points at a single static file.
- Respected the badge's 200x200 resolution ceiling everywhere. It is placed at 44 units in the lockups, which is 33 px at the 300 px login width and about 8 mm on the printed cover — a downscale in every case. Nothing places it above 96 px.
- Fixed the app-loading splash, which had still shown the Paperless-ngx leaf and "Paperless-ngx is loading...". It now shows the Vaultly mark and wording. This was a pre-existing gap, not part of the original brief.
- Corrected the Angular primary colour. `theme.scss` had `hsl(245, 65%, 51%)`, which resolves to `#3E31D3` — close to, but not, the brand indigo. It is now `hsl(245, 58%, 51%)` = `#4338CA`, so the interface and the logo use the same colour.
- Verified the patch mechanically. Fetched all twenty upstream files at the pinned commit `05e48b23166df7c7afe6f329b460b0511a89496c`, recomputed every hunk header, and confirmed `git apply --check` and `git apply` both exit 0.
- Regenerated `Vaultly-User-Guide.pdf` from the updated source: valid `%PDF-1.4` header, 24 pages, Title metadata now `Vaultly by PBSS User Guide`. Inspected the printed cover by forcing the print stylesheet on screen — the lockup fits the header with no overflow.
- Reason: give the product a more polished identity that renders consistently on every device, and introduce PBSS as a clearly secondary partner attribution.

#### Naming decisions per surface

| Surface | Name used | Why |
| --- | --- | --- |
| PDF guide title metadata | Vaultly by PBSS User Guide | Becomes the PDF's Title field; no space constraint |
| Guide cover | Lockup + `User Guide` heading | The lockup already states the full name; repeating it in the heading would be redundant |
| Guide footer and licence text | Vaultly by PBSS | Running text, no constraint |
| README | Vaultly by PBSS | Repository front door |
| Django admin header and index title | Vaultly by PBSS | Full-width headings with room to spare |
| Django admin browser title | Vaultly Admin | Tab width; the short form is more useful when truncated |
| Login page logo | Full lockup, ringed badge | The one large co-branded surface. Ringed variant because the login background is `#f5f5f5`, where the plain white disc would disappear |
| Sidebar brand | `Vaultly` + `by PBSS` byline | Reuses the upstream title/byline slot; the full name on one line would truncate in a narrow sidebar |
| `PAPERLESS_APP_TITLE` | Vaultly | Feeds the narrow sidebar slot and the browser tab |
| Page and tab titles | Vaultly | Tab width |
| Splash screen | Vaultly | Momentary screen; brevity reads faster |
| URLs and static paths | vaultly-* | Unchanged; renaming would break the existing mounts |

### 2026-07-25 — Phase 1, Step 7: frontend7 image built on VM

- Fast-forwarded the VM checkout through publication commit `766dc92`.
- Gracefully stopped only the webserver and successfully built `vaultly-paperless:2.20.15-frontend7` in approximately 55 seconds.
- Docker reused unchanged application/frontend layers and rebuilt the bundled guide/static layer; all persistent services and volumes remained intact.
- Reason: package the detailed HTML/PDF manual into the deployable Vaultly image.
...
- Fast-forwarded the VM checkout through publication commit `2e2a73f`.
- Gracefully stopped only the webserver and successfully built `vaultly-paperless:2.20.15-frontend5` in approximately 153 seconds.
- The build included the compiled Vaultly interface and local User Guide; PostgreSQL, Redis, Tika, Gotenberg, and persistent volumes remained intact.
- Reason: compile the self-contained help experience before replacing the running webserver.
