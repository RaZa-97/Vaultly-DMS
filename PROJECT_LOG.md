...
- **Branding:** The *Vaultly by PBSS* identity is applied across every surface in the inventory. The core mark was refined, the wordmark rebuilt as outlined Inter paths, and co-branded lockups added. All ten SVGs are generated by `branding/build-branding.py`.
- **Built:** `vaultly-paperless:2.20.15-frontend8` is prepared locally and not yet built on the VM. The frontend patch was verified with `git apply --check` against the pinned upstream commit `05e48b23166df7c7afe6f329b460b0511a89496c` and applies cleanly.
- **Pushed:** `origin/main` includes the refreshed branding assets and the `frontend8` revision. No VM secret or local credential file is stored in Git.
...
- **Next:** Update `PAPERLESS_APP_LOGO` in the VM's `.env` to `/static/vaultly-lockup-full.svg`, build `frontend8`, start the webserver, then verify the login page, sidebar, favicon, splash screen, and PDF guide cover.
